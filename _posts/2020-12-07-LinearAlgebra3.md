...
- ($A - \lambda I$)는 역행렬이 존재하면 안된다. 즉, $\| A - \lambda I \| = 0$이 되어야 한다.
- 이를 **고유방정식(특성방정식)**이라고 한다.
...
- 고유벡터의 경우, 위 위 방정식을 만족시키는 무슨 벡터든 가능하다.
...
2. $A$의 고유치가 $\lambda$이면 
  ...
  - A행렬의 고유치에 0이 있으면, 행렬식 값은 0이 된다.
  - 즉, 비가역 행렬이고, 이는 역행렬이 존재하지 않는다는 뜻이다.
  - 역행렬이 존재하지 않으면, 행렬식의 값은 0이다. 행렬식의 값이 0이면 역행렬이 존재하지 않는다.

## 고유치 및 고유벡터의 성질 3
1. 대칭행렬의 고유치는 항상 실수이며, 서로 다른 고유치에 해당되는 각각의 고유벡터는 수직 관계를 갖는다.
2. 교대행렬의 고유치는 0또는 순허수($i, 2i, 3i$)이다. (복소수 $a+i$의 꼴)
3. 직교행렬의 고유치는 1 또는 -1 및 켤레복소수이며, 그 절대값은 항상 '1'이다.
...
- 대각화 행렬 또한 또한 닮은 **행렬의 성질(행렬식, trace, 고유치, 계수가 같음)**을 갖는다.
